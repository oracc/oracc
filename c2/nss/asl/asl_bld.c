	     ...
	     const unsigned char *var, const unsigned char *ref)
{
  if (sl->curr_sign->forms && hash_find(sl->curr_sign->hforms, n))
  ...
  else
    {
      ...
      if (!(f = hash_find(sl->hforms, f->name)))
	{
...
void
asl_bld_value(Mloc *locp, struct sl_signlist *sl, int type, const unsigned char *n, const char *lang, const unsigned char *ref)
{
  struct sl_value *v;
  struct sl_inst *i = memo_new(sl->m_insts);
  ...
  if ((v = hash_find(sl->hsignvalues, n)))
      ...
      if (!(v = hash_find(sl->hvalues, n)))
	{
	  ...
	  /* Add inst to form's values */
	  if (sl->curr_form)
	    {
	      if (!v->fowners)
		v->fowners = list_create(LIST_SINGLE);
	      list_add(v->fowners, sl->curr_form);
	      if (!sl->curr_form->u.f->hvalues)
		sl->curr_form->u.f->hvalues = hash_create(1);
	      else if (hash_find(sl->curr_form->u.f->hvalues, n))
		{
		  mesg_verr(locp, "duplicate value %s in form %s::%s", n, sl->curr_sign, sl->curr_form);
		  return;
		}
	      hash_add(sl->curr_form->u.f->hvalues, v->name, i);
	    }
	  else
	    {

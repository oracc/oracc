...
main(int argc, char **argv)
{
  ...
  if (argc == 2)
    ...
    {
#if 0
      fprintf(stderr, "slix: must give output DB name or -boot option on command line\n");
      exit(1);
#endif
    }
  ...
  if (boot)
    {
      strcpy(dbname, "sl");
      strcpy(dbdir, "/home/oracc/pub/ogsl/sl");
      if (!(infile = fopen("/home/oracc/pub/ogsl/sl/sl.tsv", "r")))
	{
	  fprintf(stderr, "slix: boot mode requires /home/oracc/pub/ogsl/sl.tsv\n");
	  exit(1);
	}
    }
  else
    {
      ...
      if (!(infile = fopen("02pub/sl/sl.tsv", "r")))
	{
	  fprintf(stderr, "slix: can't index signlist without 02pub/sl/sl.tsv\n");
	  exit(1);
	}
      /*infile = stdin;*/
    }
  ...
  while ((k = (unsigned char *)fgets((char*)buf,20480,infile)))
	...
	{
	  fprintf(stderr, "slix: no tab in line\n");
	  exit(1);
	}
    }
  dbi_flush(dbi);

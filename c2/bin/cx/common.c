...
cgp_parse(struct cgp *c, unsigned char *s, locator *lp)
{
  ...
  if (']' == t[-1])
    {
      ...
      *t = '\0';
      while (t > s && '[' != *t)
        --t;
      if (t == s)
        mesg_vwarning(lp->file,lp->line,"syntax error in CGP: missing [ at start of GW");
      else
        {
          *t = '\0';
          c->gw = t+1;
          if (strchr((ccp)s,'[') || strchr((ccp)s,']'))
            mesg_vwarning(lp->file,lp->line,"syntax error in @entry: too many [ or ]");
          while (isspace(t[-1]))
            --t;
          *t = '\0';
        }
  ...
  if (c->pos && !cbdpos((ccp)c->pos, strlen((ccp)c->pos)))
    mesg_vwarning(lp->file,lp->line,"bad POS %s", c->pos);
}
#endif

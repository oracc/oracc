...
dbi_each (Dbi_index *dip)
{
  ...
  if (keybuf[i] != '\0')
	...
	error (NULL, "end-of-file reached while reading key from index %s", dip->i_fname);
      else
	error (NULL, "key too long (max %d) while reading from index %s", KBUFSIZ, dip->i_fname);
    }
...
#define xfread(_fn,_t,_buf,_siz,_cnt,_fp) \
			if (fread(_buf,_siz,_cnt,_fp)!=_cnt)\
			  error(NULL,"read %ld bytes failed on %s at %s:%d",_cnt*_siz,_fn,__FILE__,__LINE__)
...
dbi_flush (Dbi_index * dp)
{
  ...
  if (dp->h.entry_count)
      ...
      if (dp->type == DBI_ACCRETE)
	{
	   ...
	   * the header.
	   */
	  xfwrite (dp->i_fname, TRUE, &dp->h.data_size, sizeof (dp->h.data_size), 1, dp->i_fp);
	  direct_keys = 0;
	}
      else
	{
      ...
      for (np_index = 0; np_index < dp->h.entry_count; ++np_index)
	{
	  if (node_ptrs[np_index]->offset == 0)
	    mwarning(NULL,"key %s has 0 offset; kcount %d", 
...
#endif
  xfwrite (dp->h_fname, TRUE, &dp->h, sizeof (Dbi_index_hdr), 1, dp->h_fp);
#if FLUSH_DEBUG
  ...
  if (tmp_dp->type == DBI_ACCRETE)
      ...
      if (pd->cache_count)
	{
	  if (pd->kcount) /* we already wrote stuff to tmp */
	    {
	      xfwrite (tmp_dp->_tmp_fn, TRUE, &pd->kid, sizeof (Unsigned32), 1, 
		       tmp_dp->_tmp_fp);
	      xfwrite (tmp_dp->_tmp_fn, TRUE, &pd->cache_count, sizeof (Unsigned32), 1, 
		       tmp_dp->_tmp_fp);
	      xfwrite (tmp_dp->_tmp_fn, TRUE, pd->cache, tmp_dp->h.data_size, pd->cache_count, 
		       tmp_dp->_tmp_fp);
	      ++pd->kcount;
	  ...
	  else
	    {
	      ...
	      xfwrite (tmp_dp->i_fname, TRUE, pd->key, 1, strlen((const char *)pd->key) + 1, tmp_dp->i_fp);
	      xfwrite (tmp_dp->i_fname, TRUE, &pd->cache_count, sizeof (Unsigned32), 1, tmp_dp->i_fp);
	      xfwrite (tmp_dp->i_fname, TRUE, pd->cache, tmp_dp->h.data_size, 
...
open_bins (Unsigned16 start)
{
  ...
  for (bins_opened = 0, i = start; i < bins_count; ++i)
	...
	{
	  if (i == start)
	    error (NULL, "unable to open files for second stage of indexing");
	  --i;
	  tmp_flag = TRUE;
...
#define DBP (&bins[node_ptrs[kid]->bin])
 ...
 read_error:
  if (!feof (tmp_dp->_tmp_fp))
    error (NULL, "error reading tmp file containing index data");
  else
    return;
 write_error:
  error (NULL, "error writing index file (out of disk-space?)");
}
...
transfer_bin (Dbi_index*dip, Dbi_bin*dbp)
{
  ...
  /* write the key */
  xfwrite (dip->i_fname, TRUE, np->key, 1, strlen((const char *)np->key) + 1, dip->i_fp);

  /* write count of data entries for key */
  xfwrite (dip->i_fname, TRUE, &np->dcount, sizeof (Unsigned32), 1, dip->i_fp);
  ...
  for (i = 0; i < np->kcount; ++i)
    {
      ...
      xfread (dbp->fn, TRUE, &new, sizeof (Unsigned32), 1, dbp->fp);
      if (new > buf_len)
	{
	  buf_len = new;
	  free (buf);
      ...
      xfread (dbp->fn, TRUE, buf, dip->h.data_size, new, dbp->fp);
      xfwrite (dip->i_fname, TRUE, buf, dip->h.data_size, new, dip->i_fp);
    }
}
...
sort_and_dump_bin (Dbi_index *dip, Dbi_bin *dbp)
{
  ...
  xfread (dbp->fn, TRUE, buf, 1, dbp->size, dbp->fp);
  ...
  for (i = 0; i < nkids; ++i)
      ...
      if (NULL != di_bases[i])
	{
	  ...
	  for (di_rover = di_bases[i], di_rend = di_ends[i]; di_rover < di_rend; ++di_rover)
	    {
	      xfwrite (dip->i_fname, TRUE, di_rover->data, dsize, di_rover->count, dip->i_fp);
	    }
	}
    }
  free (buf);
  ...
  for (i = 0; i < dp->h.entry_count; ++i)
    {
      ...
      xfread (dp->_tmp_fn, TRUE, &count, sizeof (Unsigned32), 1, dp->_tmp_fp);
      if (count > buf_len)
	{
	  buf_len *= 2;
	  free (buf);
      ...
      /* read data proper */
      xfread (dp->_tmp_fn, TRUE, buf, dp->h.data_size, count, dp->_tmp_fp);
      node_ptrs[i]->offset = ftell (dp->i_fp);
      /* write key */
      xfwrite (dp->i_fname, TRUE, np->key, 1, strlen((const char *)np->key) + 1,
	       dp->i_fp);
      /* write count, ensuring correct size on 16 bit architectures */
#if defined(MSDOS)
      {
        Unsigned32 tmp = count;
        xfwrite (dp->i_fname, TRUE, &tmp, sizeof (Unsigned32), 1, dp->i_fp);
      }
#else
      xfwrite (dp->i_fname, TRUE, &count, sizeof (Unsigned32), 1, dp->i_fp);
#endif
      /* write data */
      xfwrite (dp->i_fname, TRUE, buf, dp->h.data_size, count, dp->i_fp);
    }
...
{
  xfwrite (tmp_dp->h_fname, TRUE, &((Clash *) vp)->h, sizeof (Clash_hdr), 1, tmp_dp->h_fp);
}
...
write_clash_indexes (void *vp)
{
  ...
  for (i = 0; i < ((Clash *)vp)->h.count; ++i) 
	...
	error (NULL, "invalid clash table");
    }
#endif
  xfwrite (tmp_dp->h_fname, TRUE,
...
write_hash_table ()
{
  ...
  while (i < tmp_dp->h.entry_count)
	...
	xfwrite (tmp_dp->h_fname, TRUE, &empty, sizeof (Unsigned32), 1, tmp_dp->h_fp);
...
dbi_open (const char *name, const char *dir)
{
  ...
  xfread (tmp->h_fname, TRUE, &tmp->h, sizeof (Dbi_index_hdr), 1, tmp->h_fp);
...
next_prime (Unsigned32 n)
{
  ...
  if (n == 0)
    error (NULL, "Couldn't find a prime greater than %lu", entry);
  return n;
}

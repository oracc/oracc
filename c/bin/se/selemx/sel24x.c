...
const char *curr_project = "cdli", *curr_index = "l24", *proxies_arg = "";
...
{
  if (!strncmp(name,gdl_ns_uri,strlen(gdl_ns_uri)))
  ...
  else if (!strncmp(name,norm_ns_uri,strlen(norm_ns_uri)))
	  ...
	  if (sig && *sig)
	    {
	      ...
	      f2_parse(NULL, 0, npool_copy((unsigned char *)sig,lempool), f2, NULL, NULL);
...
main (int argc, char **argv)
{
  ...
  index_dir = se_dir (curr_project, curr_index);
  dip = dbi_open("l24", index_dir);
  keys = se_file (curr_project, "l24", "key.lst");
  keysf = fopen(keys,"w");
  ...
  ce_cfg(curr_project,"l24","tr","txh",ce_byid, proxies);

...
binop_init()
{
  ...
  dtype_tab[d_lem]  = dtype_tab[d_txt] = word;
}
#endif
	...
	retry_rt:
	  ...
	  if (l24)
	    switch (ranges[RECORD].use)
	      {
#define sent_id(p) ((struct location24*)(p))->sentence_id
	      case USE_EXACT:
		if (sent_id(rt[0]) < sent_id(lft[0]))
		  continue;
		else if (sent_id(rt[0]) > sent_id(lft[0]))
		  goto move_left;
		break;
	      case USE_RANGE:
		if (((int)(sent_id(rt[0]) - sent_id(lft[0]))) 
		    < ranges[RECORD].min)
		  continue;
		else if (((int)(sent_id(rt[0]) - sent_id(lft[0]))) 
			 > ranges[RECORD].max)
		  goto move_left;
		break;
	      case USE_LAST:
	  ...
	  if (l24)
	    switch (ranges[WORD].use)
	      {
#define w_id(p) ((struct location24*)(p))->word_id
	      case USE_IGNORE:
		break;
	      case USE_EXACT:
		if (w_id(rt[0]) < w_id(lft[0]))
		  continue;
		else if (w_id(rt[0]) > w_id(lft[0]))
		  goto move_left;
		break;
	      case USE_LAST:
		if ((int)w_id(rt[0]) - (int)w_id(lft[0]) < ranges[WORD].min)
		  continue;
		else if ((int)w_id(rt[0]) - (int)w_id(lft[0]) > ranges[WORD].max)
		  goto move_left;
		else
		  {
		    record_match ();

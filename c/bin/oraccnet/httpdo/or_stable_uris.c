...
{
  char *cbd = malloc(strlen("/usr/local/oracc") + strlen(project) + strlen(lng) + 25);
  int ret;
  sprintf(cbd, "%s/pub/%s/cbd/%s/entry_ids.lst", "/usr/local/oracc", project, lng);
  ret = !access(cbd, R_OK);
  free(cbd);
  return ret;
}
...
res_is_list(const char *e)
{
  ...
  if (e)
	...
	++e;
      if (*e == 0)
	{
	  char *lst = malloc(strlen("/usr/local/oracc") + strlen("/www/lists/") + strlen(project)
				   + strlen(init) + 2);
	  int ret;
	  sprintf(lst, "%s/www/%s/lists/%s", "/usr/local/oracc", project, init);
	  ret = !access(lst, R_OK);
	  free(lst);
	  return ret;
	}

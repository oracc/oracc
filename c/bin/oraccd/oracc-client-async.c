 ...
 * Xmlrpc-c asynchronous RPC facilities.  This is the same as the simpler
 * synchronous client xmlprc_sample_add_client.c, except that it adds 3
 * different pairs of numbers with the summation RPCs going on
 * simultaneously.
 * 
 * Use this with xmlrpc_sample_add_server.  Note that that server intentionally
 * takes extra time to add 1 to anything, so you can see our 5+1 RPC finish
 * after our 5+0 and 5+2 RPCs.
 */

...
const char *g_oracc_project = NULL;	 /* cache of ORACC_PROJECT envp */
const char *g_script_dir = NULL;	 /* path to estseek.cgi */
const char *g_script_uri = "http://oracc.museum.upenn.edu/find";
...
static int realmain(int argc, char **argv){
  ...
  CBMAP *params;
  const char *rp;
  char *tmp, *wp, numbuf[NUMBUFSIZ], *slash, *tmp2;
  int i, len, ecode;
  /* set configurations */
  ...
  if (access(g_scriptname, F_OK))
    fprintf(stderr, "estseek: %s does not exist\n", g_scriptname);
  if (access(g_scriptname, R_OK))
    fprintf(stderr, "estseek: %s is not readable\n", g_scriptname);
  ...
  for(i = 0; i < cblistnum(lines); i++){
    ...
    if(cbstrfwimatch(rp, "indexname:")){
      ...
      if (access(g_tmplfile,R_OK) && g_script_dir)
	{
	  char *tmp = cbmalloc(strlen(g_scriptname) + strlen(g_tmplfile) + 1);
	  sprintf(tmp, "%s%s", g_script_dir, g_tmplfile);
	  g_tmplfile = tmp;
	}
      fprintf(stderr, "estseek: g_tmplfile=%s\n", g_tmplfile);
  ...
  if ((g_oracc_project = getenv("ORACC_PROJECT")))
    {
      char *tmp = NULL;
      ...
      tmp = malloc(strlen("http://oracc.museum.upenn.edu//find")
		   + strlen(g_oracc_project) + 1);
      sprintf(tmp, "http://oracc.museum.upenn.edu/%s/find", g_oracc_project);
      g_script_uri = tmp;
    }
  ...
  if(!g_db){
    ...
    cbglobalgc(tmp, free);
    g_tmpltext = tmp;
    fprintf(stderr, "estseek: g_indexname=%s\n", g_indexname);
...
static void showtitle(void){
  if(*p_phrase != '\0'){
    xmlprintf("Oracc Search Result: %48@", p_phrase);
  } else if(*p_attr != '\0'){
    xmlprintf("Oracc Search Result: %48@", p_attr);
  } else {
    xmlprintf("Oracc %@", g_deftitle);
  }
}
...
static void shownaviform(void){
  printf("<div class=\"oracchdr\">%s Full Text Search</div>\n",
	 g_oracc_project ? g_oracc_project : "Oracc");
  xmlprintf("<div class=\"form_navi\">\n");
  if (g_oracc_project)
    printf("<a href=\"http://oracc.museum.upenn.edu/%s\" class=\"navilink\">%s Home</a>\n",
	   g_oracc_project, g_oracc_project);
  printf("<a href=\"http://oracc.museum.upenn.edu\" class=\"navilink\">Oracc Home</a>\n");
  if(p_navi == NM_ADVANCED){
    xmlprintf("<span class=\"navivoid\">advanced</span>\n");
  } else {
    xmlprintf("<a href=\"%@?navi=%d\" class=\"navilink\">advanced</a>\n",
              g_script_uri, NM_ADVANCED);
  }
  if(p_navi == NM_HELP){
    xmlprintf("<span class=\"navivoid\">help</span>\n");
  } else {
    xmlprintf("<a href=\"%@?navi=%d\" class=\"navilink\">help</a>\n",
              g_script_uri, NM_HELP);
  }
  xmlprintf("</div>\n");
}
...
static void showresult(ESTDOC **docs, int dnum, CBMAP *hints, ESTCOND *cond, int hits, int miss,
  ...
  if(scores && scnum > 0){
    ...
    for(i = 0; i < scnum && i < g_relkeynum; i++){
      ...
      xmlprintf("<a href=\"%@?navi=%d&amp;phrase=%?&amp;attr=%?&amp;attrval=%?&amp;order=%?"
                ...
                " class=\"rword\">%@</a>",
                g_script_uri, p_navi, scores[i].word, myattr, p_attrval, p_order,
  ...
  if(clip > 0)
    xmlprintf("<a href=\"%@?navi=%d&amp;phrase=%?&amp;attr=%?&amp;attrval=%?&amp;order=%?"
              ...
              "Include %d Clipped</a>\n",
              g_script_uri, p_navi, myphrase, myattr, p_attrval, p_order,
    ...
    for(i = p_pagenum > NAVIPAGES ? p_pagenum - NAVIPAGES + 1 : 1;
      ...
      if(i == p_pagenum){
        ...
        xmlprintf("<a href=\"%@?navi=%d&amp;phrase=%?&amp;attr=%?&amp;attrval=%?&amp;order=%?"
                  ...
                  g_script_uri, p_navi, myphrase, myattr, p_attrval,
...
static void showdoc(ESTDOC *doc, const CBLIST *words, CBMAP *cnames, int detail,
  ...
  if(g_candetail)
    xmlprintf("- <a href=\"%@?navi=%d&amp;phrase=%?&amp;detail=%d&amp;perpage=%d&amp;clip=%d"
              ...
              g_script_uri, p_navi, p_similar > 0 ? "" : p_phrase, id,
  ...
  if(g_smlrvnum > 0)
    xmlprintf("- <a href=\"%@?navi=%d&amp;similar=%d&amp;perpage=%d&amp;clip=%d&amp;qxpnd=%d"
              ...
              g_script_uri, p_navi, id, p_perpage, p_clip, p_qxpnd, p_gmasks, p_prec);
  if(g_candir){
    str = cbmemdup(turi, -1);
    if((str[0] == '\\' && str[1] == '\\') ||
    ...
    for(i = 0; i < snum; i += 2){
      if(p_cinc >= 0 && p_cinc != id && i >= g_clipview * 2){
        ...
        xmlprintf("<a href=\"%@?navi=%d&amp;phrase=%?&amp;attr=%?&amp;attrval=%?&amp;order=%?"
                  ...
                  "[include]</a>",
                  g_script_uri, p_navi, p_similar > 0 ? "" : p_phrase, p_attr, p_attrval, p_order,

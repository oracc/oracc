...
<xsl:template name="call-back">
  <xsl:choose>
	...
	<h>Welcome to <xsl:value-of select="$PROJECT"/>, the Oracc Global Sign List.</h>
...
<xsl:template name="form-div">
  <div class="{$project}-form" id="{$project}div">
    <form name="{$hproject}form" id="{$hproject}form" action="/{$project}/sl" method="post" target="_top">

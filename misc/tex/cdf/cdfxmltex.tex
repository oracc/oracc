...
\ifx\@@end\@undefined
  \let\@@end\End % sjt -- we already saved TeX's \end as \End in div.tex
\fi
...
% set catcodes of low chars to 12 and high to 13.
\gdef\XML@tempa#1#2{
...
%%%WATCHME: sjt removed \UnicodeCharacter--we'll have to replace all that with
%%% something, no?
...
\IfFileExists{oracctex.cfg}
  ...
  }{\XML@warn{No File: oracctex.cfg}}


% get xmltex in catcode 12, for comparing with \jobname
\gdef\XML@tempa#1>{}
\gdef\XML@tempb{oracctex}
\xdef\XML@tempb{\expandafter\XML@tempa\meaning\XML@tempb}
%
%%% IN xmltex.tex THIS IS DEFINED AS pdfxmltex
\gdef\XML@tempc{oracctex}

...
my @xsystem_select1 = xcompute_select1('default');
my @xcustom_select1 = xcompute_select1('special');
...
warn "p2-project-data.plx: failed to load $ENV{'ORACC'}/lib/data/pll_linkids.xml"
...
print '<project-data xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:ev="http://www.w3.org/2001/xml-events"
>';
...
foreach my $tl (@translangs) {
	...
	warn "p2-project-data.plx: no longlang for $tl\n";
    }
}
print '</translangs>';
...
for (my $i = 0; $i <= $#top_l; ++$i) {
	...
	foreach my $sub (@sub_l) {
		...
		warn "p2-project-data.plx: no longlang for $sub\n";
...
print '<xoutline-sorters>', @xsystem_select1, @xcustom_select1, '</xoutline-sorters>';
...
compute_select1 {
	...
	my $labels = ORACC::XPD::Util::option("outline-$mode-sort-labels")
	    || 'Time,Genre,Place';
	my @select1 = ();
#	push @select1, "<xforms:group class=\"outline-select\" ref=\"instance('pager')/state[.='$mode']\">";
 	push @select1, "<xforms:select1 ref=\"instance('pager')/$mode-sort\">";
#	push @select1, "<xforms:label>Sort: </xforms:label>";
	push @select1, make_select1($fields,$labels);
	push @select1, "<xforms:setvalue ref=\"instance('pager')/trace\" value=\"'outline-sorter'\"/>",
	push @select1, "<xforms:action ev:event=\"xforms-value-changed\" if=\"instance('pager')/state[not(.='startup')]\">";
	push @select1, "<xforms:send submission=\"post-pager\"/>";
	push @select1, "</xforms:action>";
	push @select1, "</xforms:select1>";
#	push @select1, "</xforms:group>";
	@select1;
    } else {
	();
    }
...
xcompute_select1 {
	...
	my $labels = ORACC::XPD::Util::option("outline-$mode-sort-labels")
	    || 'Time,Genre,Place';
	my @select1 = ();
	push @select1, "<option-select mode=\"$mode\">";
	push @select1, xmake_select1($fields,$labels);
	push @select1, '</option-select>';
	@select1;
    } else {
	();
    }
...
make_select1 {
	...
	push @select1, 
	"<xforms:item><xforms:label>$label</xforms:label><xforms:value>$value</xforms:value></xforms:item>";
...
pll_link_ids {
	...
	if ($x) {
	    ...
	    warn "p2-project-data.plx: unknown link ID `$ref'\n";
	}
    }
    $link->toString(1);
}

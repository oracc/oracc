...
<xsl:template name="arg-views-nowrap">
  ...
  <xsl:for-each select="document(concat('/usr/local/oracc/xml/',$project,'/config.xml'))/*/xpd:option[@name='build-tei'='yes']">
    <li><a target="_blank" href="/{$project}/tei/{$text}.xml">TEI</a></li>
  </xsl:for-each>
  <xsl:for-each select="document(concat('/usr/local/oracc/xml/',$project,'/config.xml'))/*/xpd:option[@name='pager-custom-view']">
    <xsl:choose>

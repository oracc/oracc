...
#  <upper>,l		list number; like homophones, but finds all LAK etc.
#  
...
my $project = `$ENV{'ORACC'}/bin/xpdmanager.plx -showproj`;
...
foreach my $s ($sl->getDocumentElement()->childNodes()) {
    next unless $s->isa('XML::LibXML::Element');
    if ($s->localName() eq 'sign') {
	do_sign($s);
    }
}
...
do_sign {
    ...
    my $var = $s->getAttribute('var');

    if ($formMode) {
	warn "$id: no ~VAR for form `$sn'\n" unless $var;
	warn "$id: bad VAR `$var' for form `$sn'\n" unless $var =~ /^~[a-z]+[0-9]*$/;
    }
    ...
    foreach my $c ($s->childNodes()) {
	next unless $c->isa('XML::LibXML::Element');
	my $nodeName = $c->localName();
	if ($nodeName eq 'utf8') {
	    my $unode = $c;
	    ...
	    $values{$d} = $id;
	    $d =~ s/\d+[a-z]*$//;
	    push @{$values{$d,'l'}}, [$id,$d_orig];
	} elsif ($nodeName eq 'form') {
	    $formMode = 1;
	    do_sign($c);
	    $formMode = 0;
	}
...
#    my @res = ();
#    foreach my $c ($v->childNodes()) {
#        if ($c->isa('XML::LibXML::Element')) {
#            my $n = $c->localName();
#            if ($n eq 'det' || $n eq 'gloss') {
#                push @res, "{", $c->textContent(), "}";
#            } else {
#                die "can't handle element ", $c->localName(), "\n";
#            }
#        } else {
#            push @res, $c->textContent();
#        }

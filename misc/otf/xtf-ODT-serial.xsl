...
<!--  <xsl:call-template name="show-attr"/> -->
  <xsl:message>xtf-ODT-serial</xsl:message>
...
<!-- !SERIAL  <xsl:variable name="xlat-hdrs" select="key('tr-h-to-h',@xml:id)"/> -->
...
<!-- !SERIAL  <xsl:variable name="xlat-hdrs" select="key('tr-h-to-l',@xml:id)"/> -->
...
<!-- !SERIAL
 ...
 -->
<!-- !SERIAL  <xsl:variable name="xlat" select="key('tr-id',@xml:id)"/> -->
  <table:table-row>
    <xsl:choose>
...
<xsl:template name="surface-row">
  <xsl:if test="not(@implicit='1')">
	...
	<text:p>
      ...
      <table:covered-table-cell/>
<!-- !SERIAL      <xsl:call-template name="surface-or-column-tlat-dollar"/> -->
    </table:table-row>
  </xsl:if>
</xsl:template>
...
<!-- !SERIAL  <xsl:variable name="xlat" select="key('tr-id',@xml:id)"/> -->
<!--  <xsl:message>surface-or-column <xsl:value-of 
...
<xsl:template name="not-serial-tr-cells-span">
...
<xsl:template name="not-serial-tr-cells-innerp">
  <xsl:param name="rowspan"/>
...
<xsl:template name="not-serial-translation-cells">
...
<!--  <xsl:message>emit-table-columns=<xsl:value-of select="$count"/></xsl:message> -->
  <xsl:variable name="id-node" select="ancestor-or-self::xtf:transliteration"/>
  <table:table-columns>
    <xsl:for-each select="document('')/*/*[@name='array']/*[position() &lt; $count]">
...
<xsl:template name="not-serial-emit-xlat-covered">
...
<xsl:template name="not-serial-emit-xlat-empties">
  <xsl:variable name="top-node" 
		select="ancestor::xtf:transliteration|ancestor::xtf:composite"/>
  <xsl:variable name="tr-cols"
		select="$top-node/following-sibling::xtr:translation/@xtr:cols"/>
...
<xsl:template name="not-serial-emit-xlat-hdr">
  <xsl:param name="hdr"/>
  <table:table-cell office:value-type="string">
...
<xsl:template name="not-serial-emit-xlat-only-hdrs">
  <xsl:param name="hdrs"/>

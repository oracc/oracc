...
<xsl:template name="cat-tabs">
  <xi:include xmlns:xi="http://www.w3.org/2001/XInclude" 
	      href="/usr/local/oracc/rinap/00lib/tabset.xml"/>
</xsl:template>
...
<xsl:template match="table:table">
  <xsl:choose>
    <xsl:when test="ancestor::text:section[@text:name='catalog' or @text:name='cataloguncertain']">
    ...
    <xsl:otherwise>
      <xsl:message>table:table[@<xsl:value-of 
        select="ancestor::text:section/@text:name"/>] not handled</xsl:message>
    </xsl:otherwise>
  </xsl:choose>      
</xsl:template>

...
<xsl:stylesheet xml:id="otfODTstyler"
...
<xsl:import href="xtf-ODT.xsl"/>
<xsl:import href="gdl-ODT.xsl"/>
<xsl:import href="bib-ODT.xsl"/>
<xsl:import href="cat-ODT.xsl"/>
...
<xsl:template match="xtf:xtf">
  <xsl:apply-templates select="xtf:transliteration|xtf:score|xtf:composite"/>
</xsl:template>
...
<xsl:template match="office:document-content/office:automatic-styles">
  <xsl:copy>
    ...
    <xsl:for-each select="/*/*/office:body/office:text">
      <xsl:for-each select="xtf:xtf|*/xtf:xtf|*/*/xtf:xtf|oracc:data|*/oracc:data|*/*/oracc:data">
...
<xsl:template name="table-setup">
  ...
  <xsl:call-template name="emit-table-column-styles">
    <xsl:with-param name="count" 
		    select="2+sum(xtf:transliteration/@cols|xtf:composite|xtr:translation/@xtr:cols)"/>
  </xsl:call-template>
</xsl:template>
...
<xsl:template name="emit-table-column-styles">
  ...
  <xsl:for-each select="document('')/*/*[@name='array']/*[position() &lt; $count]">
	...
	<xsl:otherwise>
	  <xsl:value-of select="concat(15.5 div ($count - 2),'cm')"/>
	</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>

...
<xsl:param name="tatf-mode" select="true()"/>
...
<xsl:template match="x:transliterationX">
...
<xsl:template match="x:sealingX">
  <xsl:value-of select="concat('@sealing', ' ', @n, '&#xa;')"/>
  <xsl:apply-templates/>
</xsl:template>

<xsl:template match="x:objectX">
...
<xsl:template match="x:l">
  <!-- <xsl:param name="top-node" select="ancestor::x:transliteration|ancestor::x:composite"/> -->
  <xsl:value-of select="concat(@xml:id, '&#x9;', @label, '&#x9;%', $text-lang, '&#x9;')"/>
  <xsl:choose>
    <xsl:when test="true()"/> <!-- disable this for TATF for now -->
    <xsl:when test="bil">
...
<xsl:template match="x:transliteration|x:composite|x:score|x:object|x:surface|x:column|x:sealing">
  <xsl:apply-templates/>
</xsl:template>

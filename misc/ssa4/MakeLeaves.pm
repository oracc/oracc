...
#	print Dumper $$pw{'type'};
	if ($$pw{'type'} eq 'lemma') { # ${$$pw{'type'}} eq 'lemma') {
		...
		if (ORACC::SSA4::Is::is_hacked_cvn($new_leaves[0]) && is_cvn($leaves[$#leaves])) {
		    shift @new_leaves;
		}
		
		if (ORACC::SSA4::Is::is_num($new_leaves[0])) {
		    if ($$pw{'position'}) {
...
#	print STDERR "$pos\n";
	...
	if ($pos =~ /^C?V/) {
	    if (ORACC::SSA4::Is::is_finite($parse)) {
...
#		    print STDERR "adding $mform as $lpos\n";
		...
		$vform =~ s/\]C?V(?:\/[ti])?$/]/;
                $vform =~ s/^(.*?)_(?=.*?\[)//;
    		if (ORACC::SSA4::Is::is_P2($vleaves[$#vleaves])) {

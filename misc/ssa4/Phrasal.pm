package ORACC::SSA4::Phrasal;
use warnings; use strict; use open 'utf8'; use utf8;
use lib '@@ORACC@@/lib';
use Data::Dumper;
use ORACC::OSS2::Node;
use ORACC::OSS2::Log;
use ORACC::SSA4::Common;
use ORACC::SSA4::Number;
use ORACC::SSA4::Intrans;
use ORACC::SSA4::NNCnj;
use ORACC::SSA4::NNMod;
use ORACC::SSA4::CoreArgs;
...
my $empty_conj = ORACC::SSA4::MakeLeaves::make_leaf(undef,'CONJ','','*','',0,'DUMMY','','Phrasal',46);
my $empty_node = ORACC::OSS2::Node::make_empty_node();

sub
phrasal_init {
    ORACC::SSA4::Intrans::load_intrans();
}
...
map_verb_nonfinite {
	...
	$$node_map[$i] = 'V';
	my $is_intrans = ORACC::SSA4::Intrans::is_intrans(Vparse($nodes[$i]));
	my $j = $i;
	if ($j) {
	    my $found_cvn = 0;
	    my $found_dim = 0;
...
post_sentence_fixups {
    ...
    $s = fixup_ABS_X_prn($s);
    $s = case_double_prn($s);
    $s = ORACC::SSA4::CoreArgs::add_core_args($s);
    $s = drop_stop($s);
...
empty_cat {
	...
	('phrase', 
	 [ ORACC::SSA4::MakeLeaves::make_leaf(undef,'0','','*','',0,'DUMMY','','Phrasal',550) ], 
	 "NP-$e", 'Phrasal', 551);
}
...
expand_relative {
    my($r,$case) = @_;
    my @r = ();
    push @r, ORACC::SSA4::MakeLeaves::make_leaf(undef,'RPRO-1', '', '*','',0,'DUMMY','','Phrasal',609); # '0'
    my $pcase = '';
    ...
    push @r, (ORACC::OSS2::Node::make_node('clause',
			[
			 ORACC::OSS2::Node::make_node('phrase', [ ORACC::SSA4::MakeLeaves::make_leaf(undef,'*T*-1','','*','',0,'DUMMY',
												    undef, 'Phrasal',624) ], # '0'
...
sub
empty_n {
    ORACC::SSA4::MakeLeaves::make_leaf(undef,'N','','*','',0,'DUMMY','','Phrasal',637);
}
...
make_phrasal_conjp {
    ...
    my $conj_node;
    if ($ich) {
	$conj_node = ORACC::SSA4::MakeLeaves::make_leaf(undef, 'CONJP-1',0,'*ICH*','',0,'DUMMY','','Phrasal',1110);
    } else {
	$conj_node = $c[$conj];
    }
...
merge_cnj {
	...
	my $nnmod = 0;
	if ($i &&
	    ORACC::SSA4::NNCnj::is_nncnj(head_cfgw($nodes[$i-1]),
					 head_cfgw($nodes[$i]))) {
	    ...
	    unshift @ret, $n;
		 ...
		 && ($nnmod = ORACC::SSA4::NNMod::is_nnmod(head_cfgw($nodes[$i]),
							   head_cfgw($ret[0])))
	    ...
	    unshift @ret, ORACC::OSS2::Node::make_node('phrase', [ $n1, $n2 ], 'NP-MOD','Phrasal',1578);
		 ...
		 ($nnmod = ORACC::SSA4::NNMod::is_nnmod(head_cfgw($nodes[$i-1]),
							head_cfgw($nodes[$i])))

...
augment_node {
	...
	if (ORACC::SSA4::Is::is_v_finite($n)) {
	    my $v = verb_of($n);
...
#		print CLOG "CoreArgs: to add = $aug\n" if $logging && length $aug;
		my @newc = ();
		my @oldc = @{$$s{'children'}};
		if (ORACC::SSA4::Is::is_trace($oldc[0])) {
		    push @newc, shift @oldc;
		} elsif (!exp_case($oldc[0],$core{'nouncases'})
			 && ORACC::SSA4::Is::is_lt($oldc[0]) 
			 && ORACC::SSA4::Is::is_animate($oldc[0])
...
#		    print CLOG "CoreArgs: coercing LT to ERG\n" if $logging;
		...
		while ($#oldc >= 0 || $#exp >= 0) {
		    ...
		    if (ORACC::SSA4::Is::is_conjp($oldc[0]) || ($oc_case && length $oc_case)) {
...
#			    warn("coreargs+\n");
				...
				('phrase', 
				 [ ORACC::SSA4::MakeLeaves::make_leaf(undef,'0','#N','*',undef,1,undef,undef,'CoreArgs',148) ], 
				 "NP-$e",'CoreArgs',149);
			}
		    } else {
...
#			    warn("coreargs++\n");
			    ...
			    ORACC::OSS2::Node::make_node
				('phrase', 
				 [ ORACC::SSA4::MakeLeaves::make_leaf(undef,'0','#N','*',undef,1,undef,undef,'CoreArgs',160) ], 
...
cf_of {
    my $n = shift;
    my $cfgw = ORACC::SSA4::Phrasal::head_cfgw($n);
    if ($cfgw) {
	$cfgw =~ s/\[.*$//;
    }
    $cfgw;
...
verb_of {
	...
	if ($$n{'dsu'}) {
	    my $ret = $$n{'dsu'};
	    $ret =~ s/\].*$/]/;
	    $ret = ORACC::SSA4::MakeLeaves::edit_sem($ret);
	    return $ret;
	} elsif ($$n{'pos'} =~ /^V/) {
	    my $ret = $$n{'form'};
	    $ret =~ s/\#[Rr]+$//;
...
load_core_args {
    $loaded = 1;
    open(IN,'@@ORACC@@/lib/ORACC/SSA4/data/coreargs.txt') 
...
#		$noun =~ s/š/c/g;
	...
	$verb = ORACC::SSA4::MakeLeaves::edit_sem($verb);

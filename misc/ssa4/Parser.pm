...
parse {
    my ($class,$xcl,$sent) = @_;
    my ($nodes,$map) = ();
    ORACC::SSA4::Phrasal::phrasal_reinitialize();
    $sent = ORACC::SSA4::Phrasal::phrasal_merges($sent);
    ($nodes,$map) = ORACC::SSA4::Phrasal::map_sentence($sent);
    $$sent{'children'} = $nodes;
...
fixup {
	...
	unless (exists($known_unpromotable_tags{$c_label})) {
	    warn "SSA4::Parser: need promotability info on unknown tag `$c_label'\n";
	}
    }
}

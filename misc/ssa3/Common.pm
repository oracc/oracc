...
head {
	...
	if ($cref && ref($cref) eq 'ARRAY' && $#$cref >= 0) {
	    if (ORACC::SSA3::Is::is_v($n)) {
		foreach my $c (@$cref) {
		    return $c if $$c{'pos'} =~ /^V/;
		}
	    } elsif (ORACC::SSA3::Is::is_modp($$cref[0]) && $#$cref > 0) {
		head($$cref[1]);
	    } else {
		head($$cref[0]);
	    }
...
Vparse {
    my $n = shift;
    if (ORACC::SSA3::Is::is_leaf($n) && $$n{'pos'} =~ /^V/) {
	return $$n{'parse'};
    } else {
	foreach my $c (@{$$n{'children'}}) {
	    if (ORACC::SSA3::Is::is_leaf($c) && $$c{'pos'} =~ /^V/) {
		return $$c{'parse'};
	    }
	}
	undef;

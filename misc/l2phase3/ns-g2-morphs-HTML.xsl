...
<xsl:template match="cbd:entry">
  <div class="body">
    <xsl:attribute name="xml:id"><xsl:value-of select="concat(@xml:id,'.m')"/></xsl:attribute>
    ...
    <div class="ns-glo-sec" id="{@xml:id}.m">
      <div class="header">
        <h1 class="entry heading border-top">Morphological Analyses attested for
	<xsl:text> </xsl:text>
	...
	<span class="pos"><xsl:value-of select="cbd:pos"/></span>
      ...
      <xsl:for-each select="cbd:morphs/cbd:morph">
	<p class="ns-glo-sub">

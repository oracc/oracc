...
<xsl:template match="cbd:entry">
  ...
  <div class="ns-glo">
	...
	<h1>Forms:</h1>
	<xsl:call-template name="ns-glo-top-link">
	  <xsl:with-param name="nodes"  select="cbd:forms"/>
	  <xsl:with-param name="text" select="'spelling'"/>
	  <xsl:with-param name="plural" select="'different spellings'"/>
	</xsl:call-template>
	<xsl:call-template name="ns-glo-top-link">
	  <xsl:with-param name="nodes"  select="cbd:norms"/>
	  <xsl:with-param name="text" select="'normalized transcription'"/>
	</xsl:call-template>
	<xsl:call-template name="ns-glo-top-link">
	  <xsl:with-param name="nodes"  select="cbd:morphs"/>
	  <xsl:with-param name="text" select="'morphological analysis'"/>
	  <xsl:with-param name="plural" select="'morphological analyses'"/>
	</xsl:call-template>
      </div>
    </xsl:if>

    <xsl:call-template name="ns-glo-periods"/>
...
<xsl:template name="ns-glo-top-link">
  <xsl:param name="text"/>
  <xsl:param name="plural" select="''"/>
  <xsl:param name="nodes"/>
  <xsl:variable name="node-count" select="count($nodes)"/>
  <xsl:if test="$node-count > 0">
    <h2>
      <a class="icountu" 
	...
	<xsl:text> </xsl:text>
	<xsl:choose>
	  <xsl:when test="not($node-count=1)">
	    <xsl:choose>
	      <xsl:when test="string-length($plural) > 0">
...
<xsl:template name="ns-glo-periods">
    <div class="ns-glo-top">
      <h1>Attested in the following periods:</h1>

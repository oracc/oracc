...
get_info {
    ...
    if ($p{'vpr'}) {
	my $vpr_ref = $p{'vpr'};
	$morph .= ORACC::SMA2::Display::mcat($$vpr_ref{'vpr'},$$vpr_ref{'inter'},'vpr');
	$morph .= ':';
	$graph .= ORACC::SMA2::Display::gcat($$vpr_ref{'graph'},$$vpr_ref{'igraph'},$base,'vpr');
    }
    $morph .= '~';
    if ($p{'vsf'}) {
	my $vsf_ref = $p{'vsf'};
	...
	$morph .= ORACC::SMA2::Display::mcat($$vsf_ref{'vsf'},$$vsf_ref{'inter'},'vsf');
	$graph .= ORACC::SMA2::Display::gcat($$vsf_ref{'graph'},$$vsf_ref{'igraph'},$base,'vsf');
    }
    if ($p{'nsf'}) {
	my $nsf_ref = $p{'nsf'};
	$morph .= ',';
	$morph .= ORACC::SMA2::Display::mcat($$nsf_ref{'nsf'},$$nsf_ref{'inter'},'nsf');
	$cont = ORACC::SMA2::Display::auslaut($$nsf_ref{'nsf'}) || '0';
	$graph .= ORACC::SMA2::Display::gcat($$nsf_ref{'graph'},$$nsf_ref{'igraph'},$base,'nsf');
    }
    if ($p{'isf'}) {
	my $isf_ref = $p{'isf'};
	$morph .= '!';
	$morph .= ORACC::SMA2::Display::mcat($$isf_ref{'isf'},$$isf_ref{'inter'},'isf');
	$cont = ORACC::SMA2::Display::auslaut($$isf_ref{'isf'}) || '0';
	$graph .= ORACC::SMA2::Display::gcat($$isf_ref{'graph'},$$isf_ref{'igraph'},$base,'isf');
    }
    if ($cont && !$p{'backup'}) {
	$cont =~ s/=/=+/;
    }

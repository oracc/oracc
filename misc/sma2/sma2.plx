  ...
  retry:
	...
	if ($xli) {
		...
		if ($ORACC::SMA2::extended) {
		    $wstr = "\@form $f=$l $stemstr/$newbase$base +$cont #$morph ##$morph2 #:$graph\n";
		} else {
...
get_info {
    ...
    if ($p{'vpr'}) {
	my $vpr_ref = $p{'vpr'};
	$morph .= ORACC::SMA2::Display::mcat($$vpr_ref{'vpr'},$$vpr_ref{'inter'});
	$morph .= ':';
	$graph .= ORACC::SMA2::Display::gcat($$vpr_ref{'graph'},$$vpr_ref{'igraph'});
    }
    $morph .= '~';
    if ($p{'vsf'}) {
	my $vsf_ref = $p{'vsf'};
	...
	$morph .= ORACC::SMA2::Display::mcat($$vsf_ref{'vsf'});
    }
    if ($p{'nsf'}) {
	my $nsf_ref = $p{'nsf'};
	$morph .= ',';
	$morph .= ORACC::SMA2::Display::mcat($$nsf_ref{'nsf'});
	$cont = ORACC::SMA2::Display::auslaut($$nsf_ref{'nsf'}) || '0';
    }
    if ($p{'isf'}) {
	my $isf_ref = $p{'isf'};
	$morph .= '!';
	$morph .= ORACC::SMA2::Display::mcat($$isf_ref{'isf'});

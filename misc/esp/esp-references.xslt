...
<xsl:template name="references">
	<xsl:variable name="references" select=".//wm:ref"/>
	<xsl:if test="$references">
		<div id="References">
			<wm:bookmark id="references" hide-highlight="yes"/>
			<h2>References</h2>
			<ol>
				<xsl:for-each select="$references">
					<li id="highlight_ref_{position ()}">
						<wm:bookmark id="ref_{position ()}" hide-highlight="yes"/>
						<xsl:copy-of select="$referents//refer:referent[@id = current()/@referent]/node ()"/>
						<xsl:call-template name="page-numbers"/>
						<xsl:text> (</xsl:text>
						<wm:link bookmark="reflink_{position ()}" title="Jump back to reference number, above">Find in text ^</wm:link>
						<xsl:text>)</xsl:text>
					</li>
				</xsl:for-each>
			</ol>
	...
	<xsl:variable name="further-readings" select=".//wm:reading"/>
	<xsl:if test="$further-readings">
		<div id="FurtherReading">
			<wm:bookmark id="furtherreading" hide-highlight="yes"/>
